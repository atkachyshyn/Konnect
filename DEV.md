...
│   │           ├── sch_wiring.rs     # 21 tools (incl. connect_pins, power symbol embedding)
...
│   │           ├── sch_hierarchy.rs  # 14 tools (typed Sheet model, sheet CRUD + hierarchy migration + cleanup + pin lifecycle)
...
`tools/list` never carries all 215 tools (221 with the 6 meta-tools) by default. Measured against the current catalogue that listing is ~34K tokens of tool schemas, and they sit in every request for the whole task — not once per listing, which is what an earlier version of this section claimed. The starter baseline is ~2.2K.
...
- **`dispatcher_tools` (config key, default `true` for every client)**: adds three always-visible tools that reach the whole catalogue without enlarging `tools/list`:
  ...
  | tool | purpose |
  |---|---|
  | `list_available_tools` | browse all 215, names-only by default; `toolset=` or `search=` for descriptions |
...
- **19 toolsets, 215 tools** + 6 meta-tools (4 routing + 2 observability — see `tool-directory.md`)
- With `--no-dispatcher-tools`: 20 tools / ~2.2K tokens (starter kit + meta-tools)
- Default for every client: 23 tools / ~3.0K tokens (starter kit + meta-tools + 3 dispatcher tools, all 215 reachable on demand)
- Full-catalog `tools/list` (`eager_toolsets`): 221 tools (215 registered + 6 meta) / ~34K tokens
